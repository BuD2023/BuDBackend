...
//        list.sort(Comparator.comparing(NewsDto::getHitCount).reversed());
//        list = list.subList(0, 7);
        given(newsService.getNewsList(any())).willReturn(list);
        ...
        mockMvc.perform(get("/news")
                ...
                .andExpect(jsonPath("$[0].registeredAt").value(getAnyDate().toString()))
                .andExpect(jsonPath("$[0].title").value("기사 제목1"))
                .andExpect(jsonPath("$[0].link").value("http://news/url/1"))
                .andExpect(jsonPath("$[0].summaryContent").value("본문 요약 내용입니다.1"))
                .andExpect(jsonPath("$[0].mainImgUrl").value("http://news/main/img/url/1"))
                .andExpect(jsonPath("$[0].company").value("네이버뉴스1"))
                .andExpect(jsonPath("$[0].keywords").value("[\"프론트엔드\", \"백엔드\"]"))
